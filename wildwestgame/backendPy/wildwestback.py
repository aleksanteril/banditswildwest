...
travel_events = [{  #tapahtumat mitä tapahtuu matkustamisen aikana
        ...
        },
        {
            ...
            "text": "You eyed the steam train as it puffed along the tracks, its whistle loud against the quiet plains. He glanced down at Luna, her coat gleaming in the sun. 'Ain't no machine that can replace you, girl' he muttered, stroking her mane. The train may be faster, but Luna was loyal, steady, and true, something no engine could ever match.",
        ...
        },
        {
            ...
            "text": "The sun dipped low as a man approached, eyes cold. '{playerList[username].name},' he said, 'I hear you're fast. Let’s see.' I met his gaze, hand on my Colt. 'You looking for trouble?' He smiled. 'Let’s finish it.' A flash of steel and a sharp crack echoed through the street. The man collapsed, eyes wide in surprise. I holstered my Colt and wiped the dust from my {playerList[username].name}et. In the West, some scores were settled quicker than others.",
        ...
        },
        {
            ...
            "text": "You leaned against the weathered bar, the worn deck of cards in his hand. 'Might as well kill some time,' he thought, flicking the cards onto the table. The road would still be there when he was done. With a half-smirk, he dealt the first hand, ready for a game before the next long ride ahead",
        ...
        },
        {
            ...
            "text": "{playerList[username].name} muttered to himself, his stomach growling louder than the wind. 'Too bad there's no rest for the wicked in this godforsaken land'. He spurred his horse onward, the dusty trail stretching out before him, knowing the next town was still miles away.",
            "audio": "../sounds/horse_running.mp3"
        },
        {
        ...
        },
        {
            ...
            "text": "{playerList[username].name} met Luna, a fiery black mare, by chance under the blazing Wild West sun. With a gentle touch and a quiet bond, they clicked instantly. Now, Luna is {playerList[username].name}'s trusted steed, carrying him through dust storms and danger—his only companion on the endless roads.",
...
def events(username):
    ...
    if playerList[username].location == playerList[username].banditLocation:
        ...
        response = {
            ...
            "text": f"You finally track down the bandit, the tension thick as you face off. Weapons flash, the fight is intense but short. With skill and determination, you overpower them, securing your victory. Bound and defeated, the bandit has no choice but to come with you as you make your way back to claim justice.",
    ...
    if response.get("ID") == "snake": #tunnuksen avulla määritellään muokattavaa tapahtumaa
        ...
        if situation > 70: #chänssit "alt" ;) tapahtumalle
            response["text"] = f"The man stated he needed help, so you jumped off your horse and started to help him, under a second and there was a gun under your chin. He steals 200 dollars."
            playerList[username].updateMoney(-200)
        elif situation > 40:
            response["text"] = f"As I rode along the road, I spotted a man collapsed by the side, his clothes torn. He groaned, barely conscious, and I could see the pain in his eyes. Without thinking, I dismounted and knelt beside him. 'Hang on, I'll get you help,' I said, lifting him as gently as I could. Supporting him, to the nearest doctor. For your help he gave you 50 dollars."
            playerList[username].updateMoney(50)
        else:
            response["text"] = f"For a moment, my chest ached—not from judgment, but from a deep well of pity. I wondered who he was before the world weighed him down?"

    elif response.get("ID") == "wolf": #tunnuksen avulla määritellään muokattavaa tapahtumaa
        if situation > 70: #kuolematapaus
            response["text"] = f"Under the pale moon, {playerList[username].name} rode his horse through the quiet wilderness. A shadow moved—then leapt. The wolf's cunning was greater than {playerList[username].name}’s speed, and by dawn, the wild claimed its victory." #pelin lopetus tila
            response["terminaltext"] = "Death"
            playerList[username].death()
        else:
            response["text"] = f"{playerList[username].name} rode through the dusky plains, the fading sun painting the horizon in hues of orange and purple. His horse’s steady gait broke the stillness, but then a shadow moved to his left. A wolf stood there, its lean frame silhouetted against the twilight, eyes gleaming like amber fire. It didn’t snarl or retreat—just watched, calm and unafraid."
        ...
        if situation > 70: #kuolematapaus
            response["text"] = f"As you were heading off the bar, a drunken man challenged you to a brawl. But he didnt play fair, he reached for a knife. The knife was thrust forward, and in an instant, your world went dark."
            response["terminaltext"] = "Death"
            playerList[username].death()
        else:
            response["text"] = f"As you were heading off to the road, a drunken man challenged you to a brawl. The saloon was chaos—shouts, breaking glass, and the thud of fists on flesh. You ducked a wild swing, countering with a hard punch to the man’s gut, sending him staggering into a table."

    elif response.get("ID") == "indianscharging": #tunnuksen avulla määritellään muokattavaa tapahtumaa
        if situation > 70: #kuolematapaus
            response["text"] = f"It was a quiet day when the Native Americans rode into the village, eyes full of menace. They stormed through the streets, shouting orders. I tried to reach Luna, but before I could react, one of them spotted me. A single arrow hit the back of my head, and everything went dark. The wild west had claimed another life, as quickly and ruthlessly as it always did."
        ...
        if situation > 50:
            response["text"] = f"I always loved you claire, I still have nightmares of how it all ended so abruptly. I swear I'll correct the injustices of this world, evil will perish."
            response["terminaltext"] = "Lost but not forgotten."
        else:
            response["text"] = f"{playerList[username].name}, a blacksmith, lost everything to bandits. Armed with his father’s revolver, he saved a rancher, earning a horse and boots. Training hard, {playerList[username].name} became a cowboy and soon, a Wild West legend."

    elif response.get("ID") == "gamble": #tunnuksen avulla määritellään muokattavaa tapahtumaa
        if situation > 80:
            response["text"] = f"Before hitting the road again you decide to gamble a little. With a sly grin, you lay down your final card. The table fell silent, then erupted in groans. While smiling ear to ear you, collect the pot. 'Victory never felt so sweet.' You win 500 dollars."
            playerList[username].updateMoney(500)
        elif situation > 30:
            response["text"] = f"Before hitting the road again you decide to gamble a little. Devastation hits your face as you realize that there's no chance of winning. You lose 500 dollars."
            playerList[username].updateMoney(-500)
        else:
        ...
        if situation > 50 and playerList[username].money >= 500:
            response["text"] = f"After weeks on the trail, {playerList[username].name} returned, determined to pay Rafael for the custom Colt. He walked into the shop, pulled the leather pouch from his belt, and said, 'I’ve got your money.' Rafael nodded, a small smile on his face. {playerList[username].name} tipped his hat, knowing he had kept his word. You pay the 500 dollar debt."
            response["terminaltext"] = "Paying debt."
            playerList[username].updateMoney(-500)
        else:
            response["text"] = f"You ride past Rafael's gunstore, the sight of it reminding you of the debt you still owe for the custom Colt he crafted just for you. The revolver's smooth grip and polished barrel had been worth every penny, but your pockets were lighter than you'd like. You tell yourself you'll take care of it later, after a few more rides and a few more jobs. For now, you urge your horse forward, knowing Rafael will wait—just like always."

    elif response.get("ID") == "fleeing": #tunnuksen avulla määritellään muokattavaa tapahtumaa
        if situation > 70: #kuolematapaus
            response["text"] = f"As the sun set, I rode through the plains, only to find an outlaw blocking my path. 'This is my territory,' he growled. I reached for my Colt, but he was faster. A single shot rang out, and before I could react, the world went black. The wild west had claimed another."
            response["terminaltext"] = "Death"
            response["audio"] = "../sounds/pistol_shot.mp3"
            playerList[username].death()
        elif situation > 40:
            response["text"] = f"You stumble acros a young buck, tears running down his eyes as he tells he owns bandits money and that they threaten his family you decide to help him out lending the money he owns to the bandits. You give the fellow 300 dollars."
            playerList[username].updateMoney(-300)
        else:
            response["text"] = f"I lowered my head, pulling the brim of my hat down just a little further. The wind bit at my face, but I kept my eyes trained on the road ahead. Up ahead, there was a figure on the horse by the trail rough-looking, with a scowl that could sour the air. He didn’t seem to notice me at first, but I wasn’t taking chances. I nudged the horse into a slow trot, keeping my distance, my body relaxed but alert. As I passed, I kept my gaze steady, barely acknowledging him. No need to make a move; just get past him without a word. Trouble wasn’t something I needed today."

    elif response.get('ID') == 'generalstore':
        response['text'] = f"{playerList[username].name} muttered to himself, his stomach growling louder than the wind. 'Too bad there's no rest for the wicked in this godforsaken land'. He spurred his horse onward, the dusty trail stretching out before him, knowing the next town was still miles away."

    elif response.get('ID') == 'horse':
        response['text'] = f"{playerList[username].name} met Luna, a fiery black mare, by chance under the blazing Wild West sun. With a gentle touch and a quiet bond, they clicked instantly. Now, Luna is {playerList[username].name}'s trusted steed, carrying him through dust storms and danger—his only companion on the endless roads.",
